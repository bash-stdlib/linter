...
import sys
import difflib
from constants import URL_STANDARD_DOC, URL_TESTING_DOC, CACHE_FILE

class StdlibHTMLParser(html.parser.HTMLParser):
    def __init__(self):
        super().__init__()
        self.functions = set()
...
def lint_file(filepath, metadata):
    ...
    try:
        ...
        for match in stdlib_re.finditer(content):
            call = match.group(1)
            # Documentation often includes trailing underscores or dots in some contexts,
            # but our parser should have handled most.
            # If the match ends in a dot, strip it.
            if call.endswith('.'):
                call = call[:-1]
...
def main():
    ...
    if args.rebuild:
        metadata = fetch_stdlib_metadata()
        if metadata:
            save_cache(metadata)
        return
