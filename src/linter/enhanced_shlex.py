...
class EnhancedShlex(shlex.shlex):
    ...
    def __init__(
        ...
        # Protect command substitutions with quotes
        ...
        super(EnhancedShlex, self).__init__(
            io.StringIO(self._protected_str),
            posix=posix,
    ...
    def read_token(self) -> Optional[AdvancedToken]:
        ...
        is_fully_quoted = self._is_token_fully_quoted(
            restored_token, original_start, original_end
        )
    ...
    def _should_stop_scanning(
        self, state: TokenScanState, raw_token: str, protected_src: str
    ) -> bool:
        """Check if we've finished matching the token."""
        if (
            state.char_index != len(raw_token)
            or state.current_quote
            or state.is_escaped
        ):
            return False
    ...
    def _handle_escaped_char(
        self, state: TokenScanState, char: str, raw_token: str
    ) -> TokenScanState:
    ...
    def _skip_comment(self) -> None:
        ...
        while (
            self.source_ptr < len(protected_src)
            and protected_src[self.source_ptr] != "\n"
        ):
            self.source_ptr += 1
    ...
    def _scan_token_in_source(self, raw_token: str) -> TokenScanState:
        ...
        if not raw_token:
            if (
                self.source_ptr < len(protected_src)
                and protected_src[self.source_ptr] in self.quotes
            ):
                ...
                while (
                    self.source_ptr < len(protected_src)
                    and protected_src[self.source_ptr] != quote_char
                ):
                    self.source_ptr += 1
                if self.source_ptr < len(protected_src):
                    self.source_ptr += 1
        ...
        while self.source_ptr < len(protected_src):
            ...
            if state.is_escaped:
                ...
                state = self._handle_unquoted_char(
                    state, char, raw_token, escape_char, protected_src
                )
    ...
    def _map_protected_to_original_offset(self, protected_offset: int) -> int:
        ...
        return (
            self._offset_map[protected_offset]
            if protected_offset < len(self._offset_map)
            else len(self.source_str)
        )

    def _is_token_fully_quoted(
        self, token: str, start_offset: int, end_offset: int
    ) -> bool:
    ...
    def _is_ansi_c_quote_start(self, text: str, pos: int) -> bool:
        r"""Check if position is the start of an ANSI-C quote ($' or $\")."""
        return pos < len(text) - 1 and text[pos] == "$" and text[pos + 1] in ("'", '"')

    def _find_ansi_c_quote_end(
        self, content: str, start: int, quote_char: str
    ) -> "Optional[int]":
    ...
    def skip_to_newline(self) -> str:
        """Advance the lexer to next newline and return skipped content."""
        if hasattr(self.instream, "seek"):
            self.instream.seek(self.source_ptr)
        skipped_content = str(self.instream.readline())
