...
class CommentIgnores(LineIteratorBase):
    ...
    def process_line(self, line_content: str, line_num: int) -> None:
        ...
        if self._in_header:
            for code in codes:
                self.state.file_ignores[(code, line_num)] = False
        else:
            ...
            for code in codes:
                if is_same_line:
                    if line_num not in self.state.line_ignores:
                        self.state.line_ignores[line_num] = {}
                    self.state.line_ignores[line_num][(code, line_num)] = False
                else:
                    next_line = line_num + 1
                    if next_line not in self.state.line_ignores:
                        self.state.line_ignores[next_line] = {}
                    self.state.line_ignores[next_line][(code, line_num)] = False
