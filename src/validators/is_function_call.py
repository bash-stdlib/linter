...
class IsFunctionCallValidator(ValidatorBase):
    ...
    def _is_valid_call(self, call: str, offset: int) -> bool:
        """Verify if the call is a known function, an active mock, or a mock method."""
        ...
        if self.MOCK_TOKEN in call:
            ...
            method = parts[1]
            if (
                self.file_state.is_mock_active(mock_name, offset)
                and method in self.global_state.mock_methods
            ):
                return True
