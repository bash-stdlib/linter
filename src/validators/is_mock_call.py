...
from issues import STD002, STD007, STD010
...
class IsMockCallValidator(ValidatorBase):
    ...
    def check(
        ...
        if self._is_mock_system_call(call):
            return self._validate_production_usage(
                call, filepath, line, column, is_test_file
            )

        if self.MOCK_TOKEN in call:
            return self._validate_mock_object_call(
                call, filepath, line, column, offset, is_test_file
            )
    ...
    def _validate_mock_object_call(
        ...
        production_issue = self._validate_production_usage(
            call, filepath, line, column, is_test_file
        )
        if production_issue:
        ...
        if method not in self.global_state.mock_methods:
            return STD002(
                ...
                column,
                call,
                "{}{}".format(mock_name, self.MOCK_TOKEN[:-1]),
            )
