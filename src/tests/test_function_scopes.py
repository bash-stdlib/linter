import os
import unittest
from typing import Any

from linter import Linter


class TestFunctionScopes(unittest.TestCase):
    def setUp(self) -> None:
        metadata: Any = {"functions": {}, "namespaces": []}
        self.linter = Linter(metadata)
        self.asset_dir = os.path.join(
            os.path.dirname(__file__), "assets/linter/function_scopes"
        )

    def test_lint__basic_function__populates_scope(self) -> None:
        filepath = os.path.join(self.asset_dir, "basic.sh")

        self.linter.lint(filepath)
        scopes = self.linter.file_state.function_scopes

        self.assertEqual(len(scopes), 1)
        scope = scopes[0]
        self.assertEqual(scope.name, "func")
        self.assertEqual(scope.start_line, 1)
        self.assertTrue(scope.contains(2, 5))
        self.assertFalse(scope.contains(0, 0))
        self.assertFalse(scope.contains(4, 0))

    def test_lint__function_keyword__populates_scope(self) -> None:
        filepath = os.path.join(self.asset_dir, "keyword.sh")

        self.linter.lint(filepath)
        scopes = self.linter.file_state.function_scopes

        self.assertEqual(len(scopes), 1)
        scope = scopes[0]
        self.assertEqual(scope.name, "func")
        self.assertEqual(scope.start_line, 1)

    def test_lint__nested_functions__populates_nested_scopes(self) -> None:
        filepath = os.path.join(self.asset_dir, "nested.sh")

        self.linter.lint(filepath)
        scopes = self.linter.file_state.function_scopes

        self.assertEqual(len(scopes), 2)
        outer = next(s for s in scopes if s.name == "outer")
        inner = next(s for s in scopes if s.name == "inner")
        self.assertTrue(outer.contains(3, 5))
        self.assertTrue(inner.contains(3, 5))
        self.assertTrue(outer.contains(6, 5))
        self.assertFalse(inner.contains(6, 5))

    def test_lint__unclosed_function__reports_std009_error(self) -> None:
        filepath = os.path.join(self.asset_dir, "unclosed.sh")

        issues = self.linter.lint(filepath)

        self.assertEqual(len(issues), 1)
        self.assertEqual(issues[0].CODE, "STD009")

    def test_lint__quoted_braces__ignores_braces(self) -> None:
        filepath = os.path.join(self.asset_dir, "quoted_braces.sh")

        self.linter.lint(filepath)
        scopes = self.linter.file_state.function_scopes

        self.assertEqual(len(scopes), 1)
        self.assertEqual(scopes[0].end_line, 3)

    def test_lint__shadowing_functions__populates_correct_scopes(self) -> None:
        filepath = os.path.join(self.asset_dir, "shadowing.sh")

        self.linter.lint(filepath)
        scopes = self.linter.file_state.function_scopes

        self.assertEqual(len(scopes), 2)
        outer = next(s for s in scopes if s.start_line == 1)
        inner = next(s for s in scopes if s.start_line == 2)
        self.assertEqual(outer.name, "hello")
        self.assertEqual(inner.name, "hello")
        self.assertTrue(outer.contains(3, 5))
        self.assertTrue(inner.contains(3, 5))
        self.assertTrue(outer.contains(5, 5))
        self.assertFalse(inner.contains(5, 5))

    def test_lint__function_with_cmd_sub_quoted_special_chars__populates_scope(
        self,
    ) -> None:
        """Regression test for bug where special chars in quoted cmd subs broke scope detection."""
        from linter.discovery_iterators.function_scope import (
            FunctionScopeDiscoveryIterator,
        )
        from linter.state.file_state import FileLinterState
        from linter.state.global_state import GlobalLinterState
        from linter.token_iterators.shlex import ShlexTokenIterator

        content = '''test_ok() { message="$("a")" }

test_broken() { message="$("!")" }

test_multiple_specials() { x="$("&")"; y="$("|")"; z="$(";")" }
'''

        global_state = GlobalLinterState(metadata={"functions": {}, "namespaces": []})
        file_state = FileLinterState()
        iterator = ShlexTokenIterator(content)
        discovery = FunctionScopeDiscoveryIterator(global_state, file_state)

        for token in iterator:
            discovery.handle_token(token)

        scopes = file_state.function_scopes

        self.assertEqual(len(scopes), 3)
        self.assertEqual(scopes[0].name, "test_ok")
        self.assertEqual(scopes[1].name, "test_broken")
        self.assertEqual(scopes[2].name, "test_multiple_specials")

        for scope in scopes:
            self.assertNotEqual(scope.end_line, -1, f"Scope {scope.name} is not closed")
            self.assertGreater(scope.end_line, scope.start_line)


if __name__ == "__main__":
    unittest.main()
