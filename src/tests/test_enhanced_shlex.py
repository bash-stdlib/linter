...
class TestEnhancedShlexCmdSubWithQuotes(unittest.TestCase):
    """Tests for command substitutions with nested quotes.
    
    Regression tests for bug where special characters inside quoted strings
    within command substitutions were incorrectly split as separate tokens.
    """

    def test_read_token__cmd_sub_with_quoted_exclamation__returns_single_token(
        self,
    ) -> None:
        content = 'message="$("!")"'
        lexer = EnhancedShlex(content, target_chars=[])
        ...
        self.assertEqual(tokens[0], "message")
        self.assertEqual(tokens[1], '$("!")')

    def test_read_token__cmd_sub_with_quoted_pipe__returns_single_token(
        self,
    ) -> None:
        content = 'y="$("|")"'
        lexer = EnhancedShlex(content, target_chars=[])
        ...
        self.assertEqual(len(tokens), 2)
        self.assertEqual(tokens[0], "y")
        self.assertEqual(tokens[1], '$("' + '|' + '")')

    def test_read_token__cmd_sub_with_quoted_ampersand__returns_single_token(
        self,
    ) -> None:
        content = 'z="$("&")"'
        lexer = EnhancedShlex(content, target_chars=[])
    ...
    def test_read_token__cmd_sub_with_quoted_semicolon__returns_single_token(
        self,
    ) -> None:
        content = 'w="$(";")"'
        lexer = EnhancedShlex(content, target_chars=[])
        ...
        self.assertEqual(len(tokens), 2)
        self.assertEqual(tokens[0], "w")
        self.assertEqual(tokens[1], '$(";")')

    def test_read_token__cmd_sub_with_single_quoted_exclamation__returns_single_token(
        self,
    ) -> None:
        content = "message=\"$('!')\"  "
        lexer = EnhancedShlex(content, target_chars=[])
    ...
    def test_read_token__cmd_sub_nested_double_quotes__returns_single_token(
        self,
    ) -> None:
        content = 'cmd="$(echo "test")"'
        lexer = EnhancedShlex(content, target_chars=[])

        tokens = list(lexer)

        self.assertEqual(len(tokens), 2)
        self.assertEqual(tokens[0], "cmd")
        self.assertEqual(tokens[1], '$(echo "test")')

    def test_read_token__cmd_sub_triple_nested__returns_single_token(
        self,
    ) -> None:
        content = 'a="$(b="$(c)")"'
        lexer = EnhancedShlex(content, target_chars=[])
        ...
        self.assertEqual(tokens[1], '$(b="$(c)")')

    def test_read_token__cmd_sub_without_quotes__returns_as_is(
        self,
    ) -> None:
        content = 'result="$(cat file.txt)"'
        lexer = EnhancedShlex(content, target_chars=[])

        tokens = list(lexer)

        self.assertEqual(len(tokens), 2)
        self.assertEqual(tokens[0], "result")
        self.assertEqual(tokens[1], "$(cat file.txt)")

    def test_read_token__multiple_cmd_subs_with_quotes__each_single_token(
        self,
    ) -> None:
        content = 'a="$("!")" b="$("!")"'
        lexer = EnhancedShlex(content, target_chars=[])
